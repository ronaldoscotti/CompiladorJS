...
function ScopeTree(){
	...
	this.buildSymbolTable = function() {
		if(verboseMode){
			putMessage("Construindo tabela de símbolos...");
        }
		...
		function expand(node){
			...
			for(var j in node.entries){
				...
				if(!node.entries[j].isUsed){
					putWarningMessage("Variável " + node.entries[j].name + " declarada, mas nunca usada");
				}

				traversalResult +=
				"Nome: " + node.entries[j].name + " | " +
				"Valor: " + node.entries[j].value + " | " +
				"Tipo: " + node.entries[j].type + " | " +
				"Escopo: " + node.entries[j].scope + " | " +
				"É Usada: " + node.entries[j].isUsed + "\n";
			} // End for
