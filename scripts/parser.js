...
function Parser(tokenStream){
	...
	this.parse = function(){
		...
		// Report the results.
		putMessage("Análise encontrou " + errorCount + " erro(s)");
	...
	if(verboseMode){
		...
		if(currentToken.kind == TOKEN_PRINT){
			...
			if(verboseMode){
				putMessage("Analisando Identificadores.");
			} // End if
...
function match(expectedKind){
	...
	if(currentToken.kind == expectedKind){
		...
		if(verboseMode){
			putMessage("Foi encontrado um tipo inesperado de token.");
		}
		putErrorMessage("Era esperado um token do tipo : " + expectedKind + " mas foi recebido um token do tipo" + currentToken.kind + ".",currentToken.line, currentToken.position);
	} // End else
} // End match
